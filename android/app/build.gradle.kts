...
// see `android/keystore/`) when present. Falls back to the debug keystore
// (via the `signingConfigs.getByName("debug")` branch below) so `flutter
// run --release` still works on a checkout without a release keystore.
...
android {
    ...
    defaultConfig {
        ...
        applicationId = "com.origilink.app"
    ...
    buildTypes {
        release {
            // Uses the real release keystore when `android/key.properties`
            // exists; otherwise falls back to the debug keystore so
